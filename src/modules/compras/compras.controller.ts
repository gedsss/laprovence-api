...
import {
  comprasService,
  type CreateComprasSchemaDTO,
  type UpdateComprasSchemaDTO,
...
export class ComprasController {
  async createCompras(request: FastifyRequest, reply: FastifyReply) {
    const data = request.body as CreateComprasSchemaDTO
  ...
  async updateCompras(request: FastifyRequest, reply: FastifyReply) {
    const { id } = request.params as { id: string }
    const data = request.body as UpdateComprasSchemaDTO
